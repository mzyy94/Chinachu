(function() {
	...
	// 設定ボタン
	var viewReservesModalBtn = new Element('a', { className: 'right' }).insert(
		'フィルター'
	);
	
	var viewReservesModal = function() {
			...
			buttons: [
				{
					...
					onClick: function(e, btn, modal) {
						...
						var result = viewReservesForm.result();
						...
						window.location.hash = '/reserves?' + Object.toQueryString(app.query);
						//todo
					}
				}
			]
		...
		var viewReservesForm = new Hyperform({
			formWidth  : '100%',
			labelWidth : '100px',
			labelAlign : 'right',
			fields     : [
	...
	function viewReserves() {
		if (app.chinachu.reserves.length === 0) return;
		...
		contentBodyHead.insert(viewReservesModalBtn.observe('click', viewReservesModal));
