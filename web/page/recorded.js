(function() {
	...
	document.observe('chinachu:reload', function() {
		...
		document.stopObserving('chinachu:recorded', viewRecorded);
	});
	...
	// 設定ボタン
	var viewRecordedModalBtn = new Element('a', { className: 'right' }).insert(
		'フィルター'
	);
	
	var viewRecordedModal = function() {
			...
			buttons: [
				{
					...
					onClick: function(e, btn, modal) {
						...
						var result = viewRecordedForm.result();
						...
						window.location.hash = '/recorded?' + Object.toQueryString(app.query);
						//todo
					}
				}
			]
		...
		var viewRecordedForm = new Hyperform({
			formWidth  : '100%',
			labelWidth : '100px',
			labelAlign : 'right',
			fields     : [
	...
	function viewRecorded() {
		...
		contentBodyHead.insert(viewRecordedModalBtn.observe('click', viewRecordedModal));
		
		var results = [];
		var result  = {};
		
		app.chinachu.recorded.each(function(program) {
